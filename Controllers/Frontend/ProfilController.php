...
class ProfilController extends Controller
{
    public function displayProfil(int $id)
    {
        ...
        $changeMdpForm->initForm('post', '#', ['class' => 'col-12 col-sm-10 offset-sm-1 col-md-8 offset-md-2 col-lg-6 offset-lg-3 my-2 p-4'])
            ...
            ->endDiv()
            ->initDiv(['class' => 'row d-flex flex-column mb-3'])
            ->addLabelFor('newPass', 'Votre nouveau mot de passe', ['class' => 'darkBrownCol col-12 p-0 mt-3 mb-2'])
            ->initDiv(['class' => 'col-12 ms-0 ms-md-2 p-0'])
            ->addInput('password', 'newPass', ['id' => 'newPass', 'class' => 'formBorder bladeBrownBac form-control w-100'])
            ->endDiv()
            ->endDiv()
            ->initDiv(['class' => 'row d-flex flex-column mb-3'])
            ->addLabelFor('newPassConfirm', 'Confirmez votre nouveau mot de passe', ['class' => 'darkBrownCol col-12 p-0 mt-3 mb-2'])
